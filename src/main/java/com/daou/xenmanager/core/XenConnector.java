...
public class XenConnector {
    ...
    public static String connect(XenServer target) throws Exception
    {
        /*
        ...
        String result = String.format("logging in to '%s' as '%s' with password '%s'...", target.hostName, target.userName,
                target.password);
        ...
        result = result + "\n" + String.format("Session API version is %s", connection.getAPIVersion().toString());
